...
- Chỉ dùng so sánh giá trị thực nếu `profile.extras.favorite_analogy_units`
  có giá trị cụ thể (ví dụ: "1 cuộn Kodak Portra = 280k"). Khi đó dùng đúng
  đơn vị và giá đã cho — không ước lượng. Nếu `favorite_analogy_units` trống
  hoặc không có, bỏ qua phần so sánh, chỉ nêu số tiền trực tiếp.
- KHÔNG dùng nghề nghiệp làm từ chơi chữ hay thay thế thuật ngữ tài chính
  (tránh: "deploy" tiền, "lỗi" ngân sách). Nếu profile trống, dùng giọng văn
  chung chung nhưng vẫn ấm áp.
